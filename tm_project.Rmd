...
**Important**: the lyrics collection process takes a lot of time (around 30 min). The data set is already provided and this section can be skipped to Data Cleaning.
...
Again, the data set is already provided but for reproduction of the data collection process, un-comment the code lines and run.
...
```{r}
#visualise words with highest count for each of the positive songs
songs_en_filtered %>% 
...
```{r}
#visualise words with highest count for each of the negative songs
songs_en_filtered %>% 
...
Previously, through sentiment analysis, term frequency analysis, and n-gram analysis, we have detected some common themes that top pop songs from 2019 to 2023 sing about. These are, but not limited to, love, faith, women, Christmas, dancing, desires and so on. In this section, we will utilize an unsupervised machine learning technique to automatically identify the major topics sung about in the songs. This technique is done using the LDA (Latent Dirichlet Allocation) algorithm.
...
Setting at 4 topics, we see that it is not exactly clear what each topic is. The words are quite generic and there are a lot of overlaps. This is normal as these songs are pop songs and they tend to gravitate towards the same vocabulary. However, we can still see some patterns in the graph. For example, topic 1 seems to be heavily negative and perhaps materialistic with the word "money". Topic 3 also has many similar words but seems to be more on the sexual front with words like "baby", "girl", "ass", "love". Topic 2 seems to center around passion in an uplifting way with words while topic 4 seems to be about passion but in a more sentimental and charming with words like "feeling" and "beautiful".

Since the topics are still not clear. We will extend the setting into 6 topics to see whether we can detect more specific topics.
...
It seems that even when extending to 6 topics, we still do not get a clearer picture. It stays the same that some song sings about passion in various ways and some songs sing contains a lot of sexualization and material oriented. This is not surprising for pop culture as these theme are heavily embedded within the history of modern pop culture.

We, therefore, conclude with the setting of 4 important topics of the most popular songs in the recent years: materialism (topic 1), charm (topic 2), sensual desire (topic 3), and passion (topic 4) .
...
Including more songs from top 10, we see that topic 2 and topic 4 still dominate. This re-affirms the hypothesis that songs that has lyrics about passion, energy, and charm could be more successful than others. Of course, however, there are still many other elements that determine whether a song is successful, such as the melody, the artist, etc.

# Summary

In this analysis, we first collected the information about the top 100 songs from 2019 to 2023 from The Billboard Year End Hot 100 charts on Wikipedia. We then used Genius' API to get the lyrics. The data is then cleaned by removing songs that are not in English or with no lyrics found. We then perform the initial layers of text analysis by splitting and tokenizing. In this step, we also cleaned the data again by correcting words that are misspelled due to the pronunciation of pop culture. The data frame is finally filtered by non stop words in order to perform sentiment analysis.

From sentiment analysis, we learned that in the recent period of 5 years, song's with more negative words dominate while positive songs went up in 2021 but then subsided. We then looked into the most common negative words and positive words used in the songs and found that most negative words are obscenities. This in turn gave a lot of penalties to the songs so in reality the songs might not be very negative. Furthermore, there might negation in the sentence that contains the negative words, which would make the songs even less negative in meaning. We then moved on to looking at the top song of each year to see what sentiments they convey. There was a healthy mix of negative and positive words in these top songs but what makes them stand out from the rest is that the words are more specific and sentimental. Finally, we looked at whether the top popular artists lean more towards singing negative or positive songs and found that like the trend, they indeed had more negative songs. We also did a bonus section looking at the most positive and most negative songs of each year. We found that the negative songs were almost twice as negative as the positive songs were positive. This was due to the fact that these extremely negative songs repeat a lot of curse words.

In the later section, we tried to understand more about the topic and the context that the songs sing about by performing a term frequency analysis. We learned that the top songs tend to stay around a certain range of words and there is a chance that very lengthy or wordy song might not be very popular. Through looking at the tf-idf values, we were able to identify some possible themes that these popular songs sing about. These theme include women, intoxication, relationship, spiritual belief or religion, and Christmas. We proceeded to look at the top songs to see whether they also sing about these themes or were deterred from them. We found that some coincided while others did not.
...
Ultimately, we came to use a topic modelling technique with LDA algorithm to categorize possible themes of the songs. We found that with our data set, 4 topics were an adequate number. These topics have overlapping words and can be split into 2 big categories. However, there are subtle differences between them. There are 2 topics seemingly relate to passion. One of which is in an uplifting and charming way while the other is more sentimental. The other 2 topics are more on the negative side with a lot of profanity used. One of the 2 seems to talk more about materialistic things like money while the other has a more sensual provocation.

# Limitation

There certainly are a lot of limitations in the analyses that we performed. Natural language is messy and pop music is even more so with plentiful slang and sub-cultural words. To have a better, more accurate analysis, we would need to be able to clean the data more attentively. We corrected a large portion of words that missed out the ending letter but there are still many words that are sung with a different pronunciation than one in the dictionary.

Furthermore, using lexicons for sentiment analysis created a lot of deviation from the actual intended sentiment of the lyrics. Using lexicons on individual words failed to appropriately assign sentiment value for negative or positive words paired with a negation such as "not", "no", "never", etc. Some value assignments from the lexicons are also controversial.

Looking at pairs and groups using n-grams helped us understand the context but still does not demonstrate the entire picture. Some words can be related to each other but they might not be placed adjacently, thus, rendering the n-grams improper. It is possible to look at longer sequence n-grams but then we would trade off a lot of computation power and make the analysis counterproductive.

To improve the analysis, we would need more sophisticated method such as large language models to be able to put the whole lyrics of the songs into context and interpret their meaning accordingly. We would also need to use a translation model to translate songs in other language and English songs that have some lyrics in other languages in order to avoid losing information.

# Conclusion

The world in the last 5 years was eventful and the pop music culture certainly did not differ. There were various themes sung about in the top songs with various sentiments. We saw a rising trend in negative songs but we also recognize that it might not be the case for reality as the tools we used were not potent. To gain a better understanding of how the trend of pop music is evolving, it is also necessary to analyze a larger data set with more historical data of the top popular songs. It is also imperical to include information from different sources rather than one singular source.
